      ...
      <div className="grid grid-cols-6 gap-2 sm:gap-3 lg:gap-4">
        {reservationCardsData.map((card) => (
          ...
          >
            ...
            />
            <p className={`${cardStyles.textSize} font-semibold text-gray-800 group-hover:text-white transition-colors duration-300 text-center whitespace-nowrap overflow-hidden text-ellipsis max-w-full`}>{card.name}</p>
            <p className={`${cardStyles.countSize} font-bold text-gray-900 group-hover:text-white transition-colors duration-300`}>{card.count}</p>
          </div>
          ))}
      </div>
